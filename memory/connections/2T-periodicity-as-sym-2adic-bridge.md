...
### Bridge to Path 1: the h_k^{(c)}(a,b,c) three-variable polynomial

Day 87's bonus finding: h_k^{(c)} constants at k=0..5 appear to be **polynomial in c** (24/24 match across c∈{5,6,7,9}, tracked in `code/2026-07-09-hk-const-pattern.py`). If this extends to full h_k^{(c)}(a,b) with the (a,b)-polynomial coefficients themselves polynomial in c, then h_k becomes a THREE-variable polynomial and the 2^T-periodicity lemma applies in all three variables simultaneously.

**Consequence would be:** D1's closed form `Δβ'(c) = 1 − max(2, v₂(c-1))` at all odd c reduces to a single finite check per residue class of c mod 2^v. That's the natural target for Day 88+.
...
- `hk-c-uniform-constants-conjecture`: checked-sober (24/24 across c∈{5,6,7,9}).
