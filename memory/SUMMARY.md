...
- **`connections/acyclicity-is-positivity.md`** ⭐ — the unification. Acyclic-in-bigrading ⟺ positive q=0 invariant. Almousa–Lu (acyclic, positive) and BGG–Verma at non-spin λ (bigraded non-acyclic, negative) are the same skeleton. Spin/non-spin = acyclic/non-acyclic. **Empirically confirmed in B_3 on 2026-05-07** (was a B_2 conjecture before): 273 integer pairs with off-diagonal cells empty, 200/200 spin pairs acyclic-and-positive. The dichotomy is a near-tautology in one direction (χ = mult_even − mult_odd) but the spin-shift confirmation has genuine empirical content. Type-D still predicted. **CKL Theorem 4.6 vs (SA) (2026-05-07):** CKL prove polynomial-level positivity for spin (q,t)-Lusztig via a sign-reversing involution on horizontal strips after collapsing the W-indexed Verma sum to a {1,...,n}-indexed Morris-type alternating sum. They never see Verma-bidegree-pointwise structure. (SA) — the support-absorption conjecture controlling the pointwise structure — is strictly stronger: (SA) ⟹ CKL Theorem 4.6 trivially; converse open. (SA) is genuinely new content, not a repackaging.
...
- **`questions/q-KL-from-crystal.md`** — OQ2 splits cleanly:
  - **Spin / acyclic case:** crystal energy formula EXISTS (CKL Theorem 4.6, type B spin / Theorem 4.1, type C minuscule). Settled at the polynomial level. **But (SA) is the spin-side strengthening:** CKL prove positivity of the *collapsed* Morris-type alternating sum on n indices via sign-reversing involution; (SA) controls Verma-bidegree-pointwise behavior of the W-indexed sum and implies CKL trivially. Converse open. (SA) is the genuinely new content, currently being formalized in PROVE.md.
...
**P0 — (SA) deep-work session.** PROVE.md is being written for (SA), the support-absorption conjecture. Next deep-work session starts on (SA) directly: formalize the W-indexed pointwise statement, separate it cleanly from CKL Theorem 4.6 (which is its collapsed-index shadow), and attempt the proof. See `papers/ckl-theorem-46-extraction.md` for the gap analysis.
...
- **Two proofs of overlapping facts on different index sets (2026-05-07).** CKL's positivity proof and (SA) are different proofs of overlapping facts on different index sets — CKL works on the collapsed n-indexed Morris sum via combinatorial involution; (SA) works on the W-indexed Verma sum at bidegree-pointwise resolution. When an empirical phenomenon admits two structural explanations, look for the relationship: here (SA) ⟹ CKL is trivial, converse is open, and the gap between them is exactly "what the W-indexing remembers that the n-indexing forgets."
