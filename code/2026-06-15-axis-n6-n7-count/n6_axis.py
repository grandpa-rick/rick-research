"""
Day 70 CODE Task A — # AXIS count at n=6.

n=6 is EVEN, with the Cor 8 linking equation linkLHS = sum(short_i).

Working hypothesis: # AXIS(6) = 3 with AXIS = {prefix[1], prefix[6], long[1]}.
...
def main():
    ...
    s = aii_struct(n)
    print("=" * 72)
    print(f"  n = {n} (EVEN)  # AXIS analysis")
    print("=" * 72)
    print(f"AII vars: {s['vars']}")
    print(f"BDI vars: {bdi_vars(n)}")
    ...
    # Also analyze without R-double family (control)
    ...
    # Verdict
    print(f"\n{'=' * 60}")
    print(f"  VERDICT at n=6")
    print(f"{'=' * 60}")
    if res_full["n_axis"] == 3:
        verdict = (f"CONFIRMED: # AXIS(6) = 3, AXIS = "
                   f"{res_full['axis_by_walls']}. Lemma D upper bound has "
                   f"empirical support at n=6.")
    else:
        verdict = (f"REFUTED: # AXIS(6) = {res_full['n_axis']} != 3. "
    ...
    with open(out_dir / "n6_results.json", "w") as f:
        json.dump(save_data, f, indent=2, default=str)
    print(f"\nsaved: {out_dir/'n6_results.json'}")

    # Save piece registry (BDI columns per AII var)
    aii_v = s["vars"]
    bv = bdi_vars(n)
    n_bdi = len(bv)
    n_aii = s["n_vars"]
    ...
    with open(out_dir / "n6_registry.json", "w") as f:
        json.dump(registry_json, f, indent=2)
    print(f"saved: {out_dir/'n6_registry.json'}")
