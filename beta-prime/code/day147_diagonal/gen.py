"""Regenerate b_k and h_j from the Day 146 definition (core.py Psi recursion)."""
from core import *
...
t0=time.time()
P=build_P(BMAX)
t1=time.time(); print(f"build_P({BMAX}) took {t1-t0:.1f}s",flush=True)
...
t2=time.time(); print(f"H took {t2-t1:.1f}s",flush=True)
LG=slog(FP,BMAX)
t3=time.time(); print(f"log took {t3-t2:.1f}s",flush=True)
K=(BMAX+1)//3
